...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block completionBlock:(CompletionBlock)completion {
	dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
...
//    Ceilf is added as sizewithattribute method is used
	labelSize.width = MIN(ceilf(labelSize.width), maxWidth);
	totalSize.width = MAX(totalSize.width, labelSize.width);
	totalSize.height += ceilf(labelSize.height);
	if (ceilf(labelSize.height) > 0.f && indicatorF.size.height > 0.f) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.color) {
        ...
        CGFloat fillColor[4]    = {104.0/255.0,104.0/255.0,104.0/255.0,0.5};
	...
	CGRect boxRect = CGRectMake(roundf((allRect.size.width - size.width) / 2) + self.xOffset + 10,
								roundf((allRect.size.height - size.height) / 2) + self.yOffset + 10, size.width - 20, size.height - 20);
	float radius = 10.0f;
